-- 0201_app_map_volume.sql
-- Decorate the map payload with each observation's real *volume* (the size/quantity
-- attribute), so pins ramp by volume rather than computed priority weight.
--
-- WHY: the CityCrawl action-plan model no longer computes a per-point priority. Pins
-- are decorated by their own metadata (volume); spatial priority is expressed as
-- clusters. `weight` stays in the payload for legacy/risk-ROI paths but no longer
-- drives the pin styling.
--
-- Volume is derived with the SAME quantity-attribute lateral join used by
-- app_observation_detail (number_value of the unit-bearing attribute) — no new tables.
--
-- The RETURNS TABLE signature gains a column, so `create or replace` cannot be used
